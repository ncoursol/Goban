...
void create_grid(gomo_t *gomo, vec3_t position, int nb_col, int nb_row, vec3_t *stones, int nb_stones)
{
    ...
    for (int k = 0; k < nb_stones; k++) {
        initial_lines[nb_row + nb_col + k] = (line_t){
            ...
            {position.x + (stones[k].x * gap) + (gap / 2) - offset, position.y + (stones[k].y * gap) + (gap / 2) - offset, 3.2f},
            stones[k].z == 0 ? (vec3_t){0.0f, 0.0f, 1.0f} : (vec3_t){1.0f, 0.0f, 0.0f}
        };
    ...
    add_lines_batch(gomo, initial_lines, nb_row + nb_col + (nb_stones * 2), 300);
}
...
void change_tutorial(gomo_t *gomo)
{
    ...
    switch (gomo->cursor)
    {
        case 12:
            ...
            add_text_to_render(gomo, "font_text2", "the board. The goal is to be the first to make an unbroken row of five stones", (vec3_t){0.8f, 2.4f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 22);
            add_text_to_render(gomo, "font_text2", "or more (horizontal, vertical or diagonal).", (vec3_t){0.8f, 2.3f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 23);
            create_grid(gomo, (vec3_t){-2.3f, 1.8f, 3.2f}, 5, 5, (vec3_t[5]){{0, 0, 0}, {1, 1, 0}, {2, 2, 0}, {3, 3, 0}, {4, 4, 0}}, 5);
            add_text_to_render(gomo, "font_text2", "A player can also win by meeting a capture victory", (vec3_t){0.8f, 2.2f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 24);
            add_text_to_render(gomo, "font_text2", "condition (see Captures)", (vec3_t){0.8f, 2.1f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 25);

            add_text_to_render(gomo, "font_text2", "Common rules:", (vec3_t){1.0f, 1.8f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.002f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 26);
            add_text_to_render(gomo, "font_text2", "- Black moves first unless the opening rule (Swap2) changes who takes Black.", (vec3_t){0.8f, 1.6f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 27);
            add_text_to_render(gomo, "font_text2", "- Gomoku is played on a 19x19 Goban, without limit to the number of stones.", (vec3_t){0.8f, 1.5f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 28);
            add_text_to_render(gomo, "font_text2", "- There is no limitation of time.", (vec3_t){0.8f, 1.4f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 29);
            add_text_to_render(gomo, "font_text2", "- You don't stand a chance against my AI.", (vec3_t){0.8f, 1.3f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 30);
            break;
        case 13:
            ...
            add_text_to_render(gomo, "font_text2", "- Captures are resolved immediately when the flanking stone is placed.", (vec3_t){0.8f, 2.3f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 23);
            add_text_to_render(gomo, "font_text2", "- If a player captures a total of 5 pairs of stones, they win the game.", (vec3_t){0.8f, 2.2f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 24);
            add_text_to_render(gomo, "font_text2", "- Captures can change threats and block lines; they may also be used tactically", (vec3_t){0.8f, 2.1f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 25);
            add_text_to_render(gomo, "font_text2", "to prevent opponent fives.", (vec3_t){0.7f, 2.0f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 26);
            add_text_to_render(gomo, "font_text2", "- A player can place a stone even if it create a capture situation for the", (vec3_t){0.8f, 1.9f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 27);
            add_text_to_render(gomo, "font_text2", "opponent without being captured themselves.", (vec3_t){0.7f, 1.8f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 28);
            break;
        case 14:
        ...
        case 15:
            ...
            add_text_to_render(gomo, "font_text2", "- Double free-three: a single move that simultaneously creates two distinct", (vec3_t){0.8f, 2.3f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 23);
            add_text_to_render(gomo, "font_text2", "free-threes (two independent open-three threats).", (vec3_t){0.7f, 2.2f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 24);
            add_text_to_render(gomo, "font_text2", "Double free-threes are FORBIDDEN. Any move that creates two or more separate", (vec3_t){0.8f, 2.1f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 25);
            add_text_to_render(gomo, "font_text2", "open threes for the same player is illegal and must be rejected.", (vec3_t){0.8f, 2.0f, 3.2f}, (vec3_t){0.0f, PI, 0.0f}, 2, 0.0015f, (vec3_t){0.9f, 0.9f, 0.9f}, 1, 26);
            break;
        case 16:
...
void display_tutorial(gomo_t *gomo)
{
    ...
    // Clear previous tutorial texts
    clear_text_to_render(gomo, 3);
    clear_text_to_render(gomo, 4);
